...
// Добавляем \n в конец для корректной работы парсера
const code = `x = 42 + 1
y = x * 2
`;
...
watLines.push('    (local $result f64)');

for (const stmt of ast.statements) {
    if (stmt.type === 'Assign') {
        watLines.push(`    ;; ${stmt.name} = ${stmt.value.value}`);
        watLines.push(`    f64.const ${stmt.value.value}`);
        watLines.push(`    ;; store to ${stmt.name}`);
    }
}

watLines.push('    f64.const 0');
