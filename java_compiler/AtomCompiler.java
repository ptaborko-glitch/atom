// AtomCompiler.java - Полный компилятор языка Atom в WAT
import java.util.*;
import java.nio.file.*;

public class AtomCompiler {
    // Типы токенов (копируем из лексера)
    ...
    static class Lexer {
        ...
        Token nextToken() {
            ...
            if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || ch == '_') {
                String value = readIdent();
                return new Token(TOKEN_IDENT, value);
            }
    ...
    static class Parser {
        ...
        private ASTNode parsePrimary() {
            ...
            if (t.type == TOKEN_IDENT) {
                pos++;
                return ASTNode.ident(t.identValue);
            }
    ...
    static class CodeGenerator {
        ...
        String generate(ASTNode ast) {
            ...
            for (String original : varMap.keySet()) {
                String watName = varMap.get(original);
                wat.append("  (global $").append(watName).append(" (mut f64) (f64.const 0))\n");
            }
            wat.append("\n");
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 1) {
            System.err.println("Usage: java AtomCompiler <file.atom>");
            System.exit(1);
        }
