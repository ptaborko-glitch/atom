...
function parseProgram() {
    ...
    // Убеждаемся, что достигли EOF
    if (currentToken !== exports.TOKEN_EOF.value) {
        console.log('Достигнут конец, но не EOF. Принудительно читаем EOF...');
        while (currentToken !== exports.TOKEN_EOF.value && currentToken !== undefined) {
            currentToken = exports.lexer_next_token();
            console.log(`  принудительно: токен ${currentToken}`);
        }
    }
