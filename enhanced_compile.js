...
console.log('=== Компиляция с вычитанием и делением ===');
...
const x_val = tokens[2]?.value;      // 100
const y_op = tokens[7]?.token;       // SLASH
const y_right = tokens[8]?.value;    // 2
const y_val = (y_op === 5) ? x_val / y_right : 0;  // 50

const z_op = tokens[13]?.token;      // MINUS
const z_right = tokens[14]?.value;   // 10
const z_val = (z_op === 3) ? y_val - z_right : 0;  // 40

console.log(`\nВычисления:`);
console.log(`x = ${x_val}`);
console.log(`y = x / 2 = ${y_val}`);
console.log(`z = y - 10 = ${z_val}`);
...
fs.writeFileSync('enhanced.wat', wat);
console.log('\n✅ enhanced.wat создан');
