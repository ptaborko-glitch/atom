...
const outputInstance = await WebAssembly.instantiate(outputModule, {
    runtime: {
        string_char_code_at: (ptr, index) => {
            ...
            console.log(`  string_char_code_at(${ptr}, ${index}) = ${result}`);
            return result;
        },
        string_len: (ptr) => {
            const result = view[ptr];
            console.log(`  string_len(${ptr}) = ${result}`);
            return result;
        },
...
console.log('=== Детальный тест лексера ===');
console.log('1. lexer_current_char():', exports.lexer_current_char());
console.log('2. lexer_is_digit():', exports.lexer_is_digit());
console.log('3. Вызов lexer_next_token() для числа...');
const token = exports.lexer_next_token();
console.log('   Токен:', token);
console.log('4. lexer_pos после чтения числа:', exports.lexer_pos.value);
console.log('5. lexer_current_char() после числа:', exports.lexer_current_char());
console.log('6. Ещё раз lexer_next_token()...');
const token2 = exports.lexer_next_token();
console.log('   Токен 2:', token2);
