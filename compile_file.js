...
// Читаем аргумент командной строки
const filename = process.argv[2];
if (!filename) {
    console.log('Использование: node compile_file.js <файл.atom>');
    process.exit(1);
}
...
// Простой парсер для трёх строк
let pos = 0;
function current() { return tokens[pos]; }
function next() { pos++; return current(); }

// Парсим последовательность присваиваний
const vars = {};
let lastValue = 0;
...
while (pos < tokens.length && current().token !== -1) {
    ...
    if (current().token === -1) break;
    
    // Присваивание: IDENT = NUMBER
    if (current().token === 1) { // IDENT
        const name = `var${current().value}`;
        next(); // пропускаем IDENT
        if (current().token === 7) { // ASSIGN
            next(); // пропускаем =
            if (current().token === 0) { // NUMBER
