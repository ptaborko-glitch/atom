...
// Ищем все числа
const numbers = tokens.filter(t => t.token === 0);
console.log(`\nНайдено чисел: ${numbers.length}`);
console.log(`Значения: ${numbers.map(n => n.value).join(', ')}`);

// Последнее число
const lastNumber = numbers.length > 0 ? numbers[numbers.length - 1].value : 0;
