...
function nextToken() {
    if (step >= MAX_STEPS) {
        console.log('❌ Слишком много токенов');
        process.exit(1);
    }
...
function parseExpression() {
    if (currentToken === exports.TOKEN_NUMBER.value) {
        ...
        console.log(`   число1 = ${num}`);
        if (nextToken() === exports.TOKEN_PLUS.value) {
            console.log(`   оператор +`);
            if (nextToken() === exports.TOKEN_NUMBER.value) {
                const num2 = currentValue;
                console.log(`   число2 = ${num2}`);
                const result = num + num2;
                console.log(`   результат = ${result}`);
                // Проверяем, что дальше EOF
                nextToken();
                if (currentToken !== exports.TOKEN_EOF.value) {
                    console.log(`   Предупреждение: после выражения не EOF, а ${currentToken}`);
                }
                return result;
            }
        }
