...
const outputInstance = await WebAssembly.instantiate(outputModule, {
    runtime: {
        ...
        string_substring: (ptr, start, length) => {
            ...
            console.log(`  string_substring(${ptr}, ${start}, ${length})`);
            const newPtr = 200000;
            view[newPtr] = length;
...
try {
    ...
    if (substringCalled && val !== 0) {
        // Читаем строку из памяти
        const len2 = view[val];
        console.log(`  ptr len=${len2}`);
        const chars = [];
        for (let i = 0; i < len2; i++) {
            chars.push(String.fromCharCode(view[val + 4 + i]));
        }
        console.log(`  строка: "${chars.join('')}"`);
    }
} catch (e) {
