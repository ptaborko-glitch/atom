...
if (!filename) {
    console.log('Usage: node var_compiler2.js <file.atom>');
    process.exit(1);
}
...
function parseProgram() {
    ...
    while (pos < tokens.length) {
        ...
        while (tokens[pos] && tokens[pos].token === 22) {
            console.log(`  Пропускаем NEWLINE на позиции ${pos}`);
            pos++;
        }
        if (pos >= tokens.length) break;
        
        console.log(`  Парсим statement на позиции ${pos}, token=${tokens[pos].token}`);
        const stmt = parseAssignment();
        if (stmt) {
            statements.push(stmt);
            console.log(`  Добавлен statement: ${JSON.stringify(stmt)}`);
        } else {
            console.log(`  Не удалось распарсить statement`);
            break;
        }
    }
    return statements;
...
wat += '    (local $result f64)\n';

// Генерируем локальные переменные
