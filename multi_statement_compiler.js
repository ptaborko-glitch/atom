...
function parseProgram() {
    ...
    while (currentToken !== exports.TOKEN_EOF.value) {
        // Пропускаем NEWLINE между statements
        ...
        // После statement ожидаем NEWLINE или EOF
        if (!peek(exports.TOKEN_EOF.value) && !peek(exports.TOKEN_NEWLINE.value)) {
            console.log('Предупреждение: ожидался NEWLINE или EOF');
            break;
        }
    }
...
console.log('\n=== Компилятор (несколько statements) ===');
console.log('Код:');
console.log(code);
console.log('\nТокены:');
...
// Возвращаем последнюю переменную
...
console.log('\n=== Сгенерированный WAT ===');
console.log(wat);
fs.writeFileSync('multi.wat', wat);
console.log('\n✅ multi.wat создан');
console.log('\n🔧 Скомпилируйте: wat2wasm multi.wat -o multi.wasm');
console.log('🔧 Запустите: node -e "const fs = require(\'fs\'); const wasm = fs.readFileSync(\'multi.wasm\'); (async () => { const i = await WebAssembly.instantiate(wasm); console.log(i.instance.exports.main()); })()"');
