...
// Простой парсер на JavaScript, использующий лексер
console.log('\n=== Парсинг ===');

let currentToken = exports.lexer_next_token();
let pos = 0;
...
function parseExpression() {
    // Очень простой парсер выражений: идентификатор или число
    if (currentToken === exports.TOKEN_IDENT.value) {
        const name = 'variable'; // в реальности нужно сохранять значение
        nextToken();
        return { type: 'Identifier', name };
    } else if (currentToken === exports.TOKEN_NUMBER.value) {
        const value = 0; // значение нужно получать из lexer_token_value
        nextToken();
...
function parseAssignment() {
    if (currentToken === exports.TOKEN_IDENT.value) {
        const name = 'var';
        nextToken();
        if (currentToken === exports.TOKEN_ASSIGN.value) {
            nextToken();
