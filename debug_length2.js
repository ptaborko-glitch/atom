...
const outputWasm = fs.readFileSync('./output_working.wasm');
...
const outputInstance = await WebAssembly.instantiate(outputModule, {
    runtime: {
        ...
        string_substring: (ptr, start, length) => {
            console.log(`  substring CALLED: ptr=${ptr}, start=${start}, length=${length}`);
            const newPtr = 200000;
            view[newPtr] = length;
...
console.log('=== Детальная отладка lexer_read_ident ===');

let start = exports.lexer_pos.value;
console.log(`start = ${start}`);

let count = 0;
while (exports.lexer_is_alpha() > 0 && count < 20) {
    count++;
    console.log(`  итерация ${count}: pos=${exports.lexer_pos.value}, ch=${exports.lexer_current_char()}, is_alpha=${exports.lexer_is_alpha()}`);
    exports.lexer_advance();
}

let length = exports.lexer_pos.value - start;
console.log(`length = ${exports.lexer_pos.value} - ${start} = ${length}`);

if (length > 0) {
    console.log(`Вызов string_substring(${strPtr}, ${start}, ${length})...`);
    const ptr = exports.string_substring(strPtr, start, length);
    console.log(`  вернула ptr=${ptr}`);
} else {
    console.log('Длина 0, string_substring НЕ вызван');
}
