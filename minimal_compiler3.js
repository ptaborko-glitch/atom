    ...
    if (currentToken === exports.TOKEN_NUMBER.value) {
        ...
        if (currentToken === exports.TOKEN_PLUS.value) {
            ...
            if (currentToken === exports.TOKEN_NUMBER.value) {
                ...
                console.log(`  результат: ${num} + ${num2} = ${result}`);
                return result;
            }
        }
        return num;
...
console.log('\n=== WAT ===');
console.log(wat);
fs.writeFileSync('minimal.wat', wat);
console.log('\n✅ minimal.wat создан');
console.log('🔧 Выполните: wat2wasm minimal.wat -o minimal.wasm');
console.log('🔧 Затем: node -e "require(\"fs\").readFileSync(\"minimal.wasm\").then(m => WebAssembly.instantiate(m)).then(i => console.log(i.instance.exports.main()))"');
