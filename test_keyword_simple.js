...
const imports = {
    runtime: {
        string_char_code_at: (ptr, index) => {
            ...
            console.log(`  char_at(${index}) = ${result} (${String.fromCharCode(result)})`);
            return result;
        },
...
console.log('=== Тест "if" с отладкой ===');
console.log('Вызов lexer_next_token...');
const token = exports.lexer_next_token();
console.log(`Результат: ${token}`);
console.log(`Ожидалось: 27 (TOKEN_KW_IF)`);
