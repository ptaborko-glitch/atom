...
exports.TOKEN_KW_ELSE.value = 28;

exports.CHAR_EOF.value = -1;
exports.CHAR_SPACE.value = 32;
exports.CHAR_TAB.value = 9;
...
function parsePrimary() {
    ...
    if (peek(exports.TOKEN_LPAREN)) {
        eat(exports.TOKEN_LPAREN);
        const expr = parseExpression();
        eat(exports.TOKEN_RPAREN);
        return expr;
    }
...
function parseComparison() {
    ...
    while (peek(exports.TOKEN_EQ.value) || peek(exports.TOKEN_LT.value) || peek(exports.TOKEN_GT.value)) {
        const op = currentToken;
        eat(op);
        const right = parseAddition();
        let operator;
        if (op === exports.TOKEN_EQ.value) operator = '==';
        else if (op === exports.TOKEN_LT.value) operator = '<';
        else operator = '>';
...
function parseIf() {
    eat(exports.TOKEN_KW_IF.value);
    const condition = parseExpression();
    eat(exports.TOKEN_COLON.value);
    eat(exports.TOKEN_NEWLINE.value);
    eat(exports.TOKEN_INDENT?.value || 4); // Упрощённо
    const thenBody = [];
    while (!peek(exports.TOKEN_DEDENT?.value) && !peek(exports.TOKEN_EOF.value)) {
        if (peek(exports.TOKEN_NEWLINE.value)) eat(exports.TOKEN_NEWLINE.value);
        else thenBody.push(parseStatement());
    }
    eat(exports.TOKEN_DEDENT?.value || 0);
    let elseBody = [];
    if (peek(exports.TOKEN_KW_ELSE.value)) {
        eat(exports.TOKEN_KW_ELSE.value);
        eat(exports.TOKEN_COLON.value);
        eat(exports.TOKEN_NEWLINE.value);
        eat(exports.TOKEN_INDENT?.value || 4);
        while (!peek(exports.TOKEN_DEDENT?.value) && !peek(exports.TOKEN_EOF.value)) {
            if (peek(exports.TOKEN_NEWLINE.value)) eat(exports.TOKEN_NEWLINE.value);
            else elseBody.push(parseStatement());
        }
        eat(exports.TOKEN_DEDENT?.value || 0);
    }
...
function parseStatement() {
    if (peek(exports.TOKEN_KW_IF.value)) return parseIf();
    if (peek(exports.TOKEN_IDENT.value)) return parseAssignment();
