...
function parseAssignment() {
    ...
    if (peek(exports.TOKEN_ASSIGN.value)) {
        ...
        // Проверяем, что после выражения достигнут EOF
        if (!peek(exports.TOKEN_EOF.value)) {
            throw new Error(`Expected EOF, got ${currentToken}`);
        }
...
try {
    ...
    console.log('\n✅ compiled.wat сохранён');
    console.log('\n🔧 Выполните: wat2wasm compiled.wat -o compiled.wasm && node -e "require(\"fs\").readFileSync(\"compiled.wasm\").then(m => WebAssembly.instantiate(m)).then(i => console.log(i.instance.exports.main()))"');
} catch (e) {
