...
function eat(expected) {
    if (currentToken !== expected) {
        throw new Error(`Expected token ${expected}, got ${currentToken}`);
    }
    return nextToken();
}
...
function parseIdentifier() {
    const name = String.fromCharCode(currentValue);
    eat(exports.TOKEN_IDENT.value);
    return { type: 'Identifier', name };
}

function parsePrimary() {
    if (peek(exports.TOKEN_NUMBER.value)) {
        return parseNumber();
    }
    ...
    throw new Error(`Unexpected token in primary: ${currentToken}`);
}
...
function parseProgram() {
    ...
    while (!peek(exports.TOKEN_EOF.value)) {
        ...
        // После statement может быть NEWLINE
        if (peek(exports.TOKEN_NEWLINE.value)) {
            eat(exports.TOKEN_NEWLINE.value);
        }
    }
