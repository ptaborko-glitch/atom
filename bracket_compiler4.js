...
function parsePrimary() {
    const t = tokens[pos];
    console.log(`  parsePrimary: pos=${pos}, token=${t.token}`);
    if (t.token === 0) {
        pos++;
        console.log(`    число: ${t.value}`);
        return t.value;
    }
    if (t.token === 14) { // LPAREN
        console.log(`    открывающая скобка`);
        pos++;
        const expr = parseExpression();
        console.log(`    выражение в скобках = ${expr}`);
        if (tokens[pos] && tokens[pos].token === 15) { // RPAREN
            console.log(`    закрывающая скобка`);
            pos++;
        }
        return expr;
    }
...
function parseMultiplication() {
    ...
    console.log(`  parseMultiplication: left=${left}, pos=${pos}`);
...
function parseAddition() {
    ...
    console.log(`  parseAddition: left=${left}, pos=${pos}`);
    while (pos < tokens.length) {
        const t = tokens[pos];
        if (t.token === 2) { // PLUS
            console.log(`    оператор +`);
