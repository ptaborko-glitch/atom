...
const imports = {
    runtime: {
        string_char_code_at: (ptr, index) => {
            ...
            console.log(`  string_char_code_at(${ptr}, ${index}) = ${result} (${String.fromCharCode(result)})`);
            return result;
        },
        ...
        string_substring: (ptr, start, length) => {
            ...
            console.log(`  string_substring создала строку длиной ${length} по адресу ${newPtr}`);
            return newPtr;
        },
...
console.log('Результат token:', token);
console.log('Токен:', token === 27 ? 'KW_IF ✅' : token === 1 ? 'IDENT' : token);
